...
test_that("Expect error if internet connection is down", {
  testthat::expect_error(
    httptest::without_internet(
      result <- ReLTER::get_site_general(deimsid = 'https://deims.org/f30007c4-8a6e-4f11-ab87-569db54638fe')
    ),
    "GET"
  )
})
...
test_that("Output of site general info function constructs ‘tibble’ as expected", {
  result <- ReLTER::get_site_general(deimsid = 'https://deims.org/f30007c4-8a6e-4f11-ab87-569db54638fe')
  ...
  expect_true(all(names(result) == c(
    ...
    "generalInfo.abstract", "generalInfo.keywords", "generalInfo.status.label", "generalInfo.status.uri", "generalInfo.yearEstablished", "generalInfo.yearClosed", "generalInfo.hierarchy.parent", "generalInfo.hierarchy.children",
...
test_that("Wrong input (but URL) constructs a tibble with empty data", {
  result <- ReLTER::get_site_general(deimsid = 'https://deims.org/ljhnhbkihubib')
  expect_type(result, "NULL")
})

test_that("Wrong input (not URL) constructs an empty tibble", {
  result <- ReLTER::get_site_general(deimsid = 'ljhnhbkihubib')
  expect_type(result, "NULL")
})
