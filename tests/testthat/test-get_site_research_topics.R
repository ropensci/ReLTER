...
test_that("Expect error if internet connection is down", {
  testthat::expect_error(
    httptest::without_internet(
      result <- ReLTER::get_site_research_topics(deimsid = 'https://deims.org/f30007c4-8a6e-4f11-ab87-569db54638fe')
    ),
    "GET"
  )
})
...
test_that("Wrong input (but URL) constructs a tibble with empty data", {
  result <- ReLTER::get_site_research_topics(deimsid = 'https://deims.org/ljhnhbkihubib')
