...
test_that("map_occ2 returns list with appropriate names", {
  ...
  suppressWarnings({
    ...
    gres_obis <- get_site_speciesOccurrences(
      deimsid = govid,
      list_DS = c("obis"),
      show_map = F
    )
...
test_that("save_occ_eLTER_reporting_Archive saves zip archive", {
  ...
  expect_error(
    save_occ_eLTER_reporting_Archive(
      res,
      path = paste0(tempdir(), "/dummy/dummy")
    )
  )
...
test_that("elter_reporting_produce_data_object
          outputs list with expected slot names", {
