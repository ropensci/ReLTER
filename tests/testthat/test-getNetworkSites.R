...
test_that("Expect error if internet connection is down", {
  testthat::expect_error(
    httptest::without_internet(
      result <- ReLTER::getNetworkSites(networkDEIMSID = "https://deims.org/network/7fef6b73-e5cb-4cd2-b438-ed32eb1504b3")
    ),
    "GET"
  )
})
...
test_that("Wrong input (but URL) constructs a NULL object", {
  result <- ReLTER::getNetworkSites(networkDEIMSID = "https://deims.org/network/ljhnhbkihubib")
  expect_type(result, "NULL")
})

test_that("Wrong input (not URL) constructs an empty tibble", {
  result <- ReLTER::getNetworkSites(activityid = "ljhnhbkihubib")
  expect_type(result, "NULL")
})

test_that("Output of get activities information function constructs ‘sf' with valid geometries", {
  result <- ReLTER::getNetworkSites(
