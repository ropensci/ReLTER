...
test_that("Expect error if internet connection is down", {
  testthat::expect_error(
    httptest::without_internet(
      result <- ReLTER::getNetworkEnvCharacts(networkDEIMSID = "https://deims.org/network/7fef6b73-e5cb-4cd2-b438-ed32eb1504b3")
    ),
    "GET"
  )
})
...
test_that("Wrong input (but URL) constructs a NULL object", {
  result <- ReLTER::getNetworkEnvCharacts(networkDEIMSID = 'https://deims.org/network/ljhnhbkihubib')
...
test_that("Wrong input (not URL) constructs an empty tibble", {
  result <- ReLTER::getNetworkEnvCharacts(networkDEIMSID = 'ljhnhbkihubib')
  expect_true(is.null(result))
  expect_true(is.null(ncol(result)))
  expect_true(length(result) == 0)
})
