...
test_that("Expect error if internet connection is down", {
  testthat::expect_error(
    httptest::without_internet(
      result <- ReLTER::getSiteRelatedResources(
...
test_that("Output of site related resources function constructs ‘tibble’ as expected", {
  result <- ReLTER::getSiteRelatedResources(deimsid = 'https://deims.org/f30007c4-8a6e-4f11-ab87-569db54638fe')
  ...
  expect_type(result$relatedResources, "list")
...
test_that("Wrong input (but URL) constructs a tibble with empty data", {
  result <- ReLTER::getSiteRelatedResources(deimsid = 'https://deims.org/ljhnhbkihubib')
  expect_type(result, "NULL")
})

test_that("Wrong input (not URL) constructs an empty tibble", {
  result <- ReLTER::getSiteRelatedResources(deimsid = 'ljhnhbkihubib')
  expect_type(result, "NULL")
})
