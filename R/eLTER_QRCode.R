...
#' @export
#' @examples
#' eLTER_QRCode(deimsid = 'https://deims.org/f30007c4-8a6e-4f11-ab87-569db54638fe')
#'
### function getResearchTopics
eLTER_QRCode <- function(deimsid) {
  require(qrencoder)
  siteTitle <- ReLTER::getSiteGeneral(deimsid = deimsid) %>% dplyr::select(title) %>% as.character()
  outputImageQRCode <- paste0("eLTER_qrCode_", gsub(' ', '_', siteTitle), ".png")
  par(mar = c(0,0,0,0))
  png(filename = paste0(outputImageQRCode))
  image(qrencoder::qrencode_raster(as.character(deimsid)), 
        asp = 1, col = c("white", "#1479BC"), axes = FALSE, 
        xlab = "", ylab = "")
  dev.off()
}
