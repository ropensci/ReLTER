#' @title eLTER_add_taxon_id
#' @description This function provide a taxon ID, usually a \href{https://en.wikipedia.org/wiki/LSID}{LSID}, to a taxon. The input of the function is a csv file with a list
#' of taxon and the provider is currently A Pan-European Species directories Infrastructure - \href{http://eu-nomen.eu/pesi/}{PESI}.
#' `taxonID` takes advantage of taxize's `eubon_search` function \url{https://docs.ropensci.org/taxize/} and the \href{http://www.eu-nomen.eu/portal/rest/}{PESI RestAPI}.
...
#' )
#'
#' # We perform a query from the `importedDataset` in order to associate the taxon identifier (LSID) to the list of species present in the thirth colum.
...
taxonIDPesi <- function (table, taxaColumn) {
    ...
    while (i <= length(importedDataset[[taxaColumn]])) {
      a <- taxize::eubon_search(
        query = importedDataset[[taxaColumn]][i],
        providers = "pesi"
      )
      if (length(a) == 0) {
        i <- i + 1
      } else if (length(a[[1]]) == 1) {
        importedDataset$canonicalName[[i]] <- a$taxon.taxonName.canonicalName
        importedDataset$authorship[[i]] <- a$taxon.taxonName.authorship
        # importedDataset$synonyms[[i]] <- a$synonyms
        importedDataset$LSID[[i]] <- a$taxon.identifier
        importedDataset$url[[i]] <- a$taxon.url
        importedDataset$accordingTo[[i]] <- a$taxon.accordingTo
        importedDataset$checkStatus[[i]] <- a$taxon.taxonomicStatus
        i <- i + 1
      } else if (length(a[[1]]) > 1) {
        ...
        # e.g. for this record importedDataset$scientificName[53] the result is:
        # matchingNameType
        # SYNONYM
        # SYNONYM
        # TAXON
