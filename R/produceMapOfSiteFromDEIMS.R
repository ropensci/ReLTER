...
produceMapOfSiteFromDEIMS <- function(deimsid, countryCode, listOfSites,
                                      gridNx, gridNy, width, height, siteName,
  ...
  siteSelected <- sf::as_Spatial(
    sf::st_as_sfc(
      jsonlite::fromJSON(paste0(
        "https://deims.org/",
        "api/sites/",
        substring(deimsid, 19)))$attributes$geographic$coordinates
    ),
  )
  ...
  geoBonBiome <- jsonlite::fromJSON(paste0(
    "https://deims.org/",
    "api/sites/",
    substring(deimsid, 19)))$attributes$environmentalCharacteristics$geoBonBiome
  ...
  if (is.null(geoBoundaries)) {
    ...
    lterSitesFeaturePointDEIMS@proj4string <- sp::CRS(
    ...
    mapOfSite <-
      ...
      tmap::tm_credits("Leaflet | © OpenStreetMap contributors -
    ...
    lterSitesFeatureDEIMS@proj4string <- sp::CRS(
      "+proj=longlat +datum=WGS84 +no_defs"
    )
    bboxlterItalySitesFeature <- sp::bbox(lterSitesFeatureDEIMS)
    bboxlterItalySitesFeature[1] <- sp::bbox(lterSitesFeatureDEIMS)[1] +
      bboxXMin
    bboxlterItalySitesFeature[3] <- sp::bbox(lterSitesFeatureDEIMS)[3] +
      bboxXMax
    bboxlterItalySitesFeature[2] <- sp::bbox(lterSitesFeatureDEIMS)[2] +
      bboxYMin
    bboxlterItalySitesFeature[4] <- sp::bbox(lterSitesFeatureDEIMS)[4] +
      bboxYMax
    ...
    print(
      mapOfCentroids,
      vp = grid::viewport(gridNx, gridNy, width = width, height = height)
    )
  }
}
