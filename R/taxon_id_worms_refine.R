...
taxon_id_worms_refine <- function(input, taxaColumn = 1) {
  for (i in 1:nrow(input)) {
    if (input$nOfWormsResults[[i]] > 1) {
      ...
      my.selection <-
        readline(prompt = "\nPlease select the record that you think most similar to the taxa name that you have provided. Insert the number of record: ")
      ...
      input$wormsRecords[[i]] <- list(input$wormsRecords[[i]][as.integer(my.selection), ])
